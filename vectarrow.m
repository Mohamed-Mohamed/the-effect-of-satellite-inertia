function vectarrow(p0,p1,linewidth,color)
...
%   $Revision: 1.0
  ...
  if max(size(p0))==3
      if max(size(p1))==3
          ...
          plot3([x0;x1;hu(:)],[y0;y1;hv(:)],[z0;z1;hw(:)],'color',color,'LineWidth',linewidth)  % Plot arrow head
      else
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          plot([x0;x1;hu(:)],[y0;y1;hv(:)],'color',color,'LineWidth',linewidth)  % Plot arrow head
      else
          error('p0 and p1 must have the same dimension')
      end
  else
